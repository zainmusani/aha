...
import {
  confirmForgetPasswordOtpRequest,
...
function EmailVerification(props) {
  ...
  const {isComingFromForgotPass} = props;
  ...
  const handleResetOTP = () => {
    ...
    if (isComingFromForgotPass) {
      ...
      dispatch(
        signUpResendOTPRequest(payload, res => {
          if (res.status) {
            setResendOTPLoader(true);
            setResetCountdownId(Math.random());
            setLoadingOTP(false);
  ...
  const onSubmitPress = () => {
    if (validation()) {
      ...
      if (isComingFromForgotPass) {
        dispatch(
          confirmForgetPasswordOtpRequest(payload, res => {
            setLoading(false);

            if (res.status) {
              Actions.replace('resetPassword', {
                forgotToken: res.data.forget_password_token,
        ...
        dispatch(
          confirmSignupOtpRequest(payload, status => {
            setLoading(false);

            if (status) {
              Actions.replace('beAnArtistOnboarding', {
                userContact: {...userContact},
              });
            }
          }),
  ...
  const imageBottomSection = () => {
    return (
      <>
        <Text style={styles.verifyEmailText}>{strings.VERIFY_EMAIL}</Text>
  ...
  const wrongEmailText = () => {
    return (
      <TouchableOpacity onPress={() => Actions.pop()}>
        <Text style={styles.wrongEmailText}>{strings.WRONG_EMAIL}</Text>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <ImageBackground
      ...
      <KeyboardAwareScrollViewComponent scrollEnabled={true}>
        {bannerImage}
        {/* <Text>{`    Your OTP is: ${incomingOTPFromScreens}`}</Text> */}
        {imageBottomSection()}
        {codeInputView()}
...
export default connect(mapStateToProps, actions)(EmailVerification);
